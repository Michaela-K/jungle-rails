...
CypressRails.hooks.before_server_start do #runs at the start of cypress. By creating a category and two products, they will populate the test database.
...
CypressRails.hooks.before_server_stop do  # will run when we close Cypress. If we forget to clean the database, products will be added everytime we start the test.
  # Called once, at_exit
end

private

def open_asset(file_name)  #To load the assets properly
